...
from .model import (OWNER_CONST, GROUP_TYPE, Group, Node, Call, Variable,
                    BaseLanguage, djoin, ImportItem, create_fake_node,create_fake_group,
                   _find_links_for_call, flatten, ShareFlag)

# from model import (OWNER_CONST, GROUP_TYPE, Group, Node, Call, Variable,
#                     BaseLanguage, djoin, ImportItem, create_fake_node,create_fake_group,
#                    _find_links_for_call, flatten)

def _deal_call_and_variable_pt_call(node):
    """
    let variable's points to Call is the exact call in the node(not a copy), for future data transfer
    and set master node
    """
    assert type(node) == Node

    #将variable.points_to指向的call对应到node.calls
...
def find_file_group_from_here(here, path_name, file_dic, level=0):
    """
    Given a target absolute(level == 0) / relative(level != 0) path from the node/group(here),
    find the file group refer to the path.

    :param here Node/Group
    :param path_name str or None
    :param file_dic ...
    :param level int
    ...
    def look_up(regular_name_from_root, file_dic):
        ...
        # print("cannot find ", regular_name_from_root, " or ", init_name_from_root)
        return None
...
def get_importitems_ImportFrom(el, node_or_group, file_dic):
    """
    1. from module/package import *，
    2. from package import module/subpackage
    3. from module/package import class/function(/variable)
    """
    ...
    for single_import in el.names:
        ...
        if file_group != None:
            ...
            if len(target_nodes_or_groups) == 0:
                # print("cannot find ", target_name, " in ", file_group.name_from_root)
                continue
...
def find_node_or_group_in_file_group(node_or_group_name, file_group, file_dic,
    ...
    if type(file_group)!=Group:
        # print(type(file_group))
        # print(file_group)
        # print(file_group.name_from_root)
        return []
    ...
    for global_varibale in file_group.global_variables:
        if global_varibale.token == node_or_group_name:
            if type(global_varibale.points_to) == str:
                ...
                # if not (type(global_varibale.points_to)==list and len(global_varibale.points_to)>0):
                #     print("here")
...
def find_node_or_group(name_from_root, file_dic):
    ...
    suffix = name_from_root[len(file_name_from_root):] #".x, .classA, .classA.b"

    if suffix.count(".") == 1 :
        func_or_class_token = suffix[1:]

        # 2.2 name_from_root represent a file-level function
        for node in file_group.nodes:
            if node.token == func_or_class_token:
                assert node.name_from_root == name_from_root
                res.append(node)

        # 2.3 name_from_root represent a class
    ...
    # 2.4 name_from_root represent a class-level function(method)
    else:
...
def find_ast_tree(node_or_group, file_dic):
    ...
    """
    Given a Node or Group object, return its ast tree.
    """
    ...
    if type(node_or_group) == Group:
        ...
        parent_ast = find_ast_tree(node.parent, file_dic) #parent can be file or class
